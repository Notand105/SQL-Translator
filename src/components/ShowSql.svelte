...
<div class="bg-gray-200 p-8 rounded-xl w-fit ">
    ...
    <code class="text-zinc-800">
       ...
       {#if entitie.PK.length == 1}
        <p>{entitie.PK[0].name} <span class="text-sky-800">{entitie.PK[0].data} PRIMARY KEY,</span></p>
       {/if}
       {#if entitie.PK.length > 1}
       ...
       {/each}  
       {/if}
       {#each entitie.atributes as atr}
        <p>{atr.name} <span class="text-sky-800">{atr.data},</span> </p>
       {/each}
        {#if entitie.PK.length > 1}
        <p><span class="text-sky-800">PRIMARY KEY</span> (
       {#each entitie.PK as pks, index }
       <p class="inline-block">{pks.name}
